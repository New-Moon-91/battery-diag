#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""w5 [5] — results/w5/*.json → summary_w5.csv

    python scripts/make_summary_w5.py
...
def main():
    ...
    with open(OUT/'summary_w5.csv', 'w', newline='', encoding='utf-8') as fh:
        w = csv.DictWriter(fh, fieldnames=FIELDS); w.writeheader(); w.writerows(rows)
    print(f'{OUT/"summary_w5.csv"}  ({len(rows)}행)')
    ...
    with open(OUT/'policy_structure_w5.csv', 'w', newline='', encoding='utf-8') as fh:
        w = csv.DictWriter(fh, fieldnames=list(pr[0])); w.writeheader(); w.writerows(pr)
    print(f'{OUT/"policy_structure_w5.csv"}  ({len(pr)}행)')
