...
## 5.2 규모 전이와 W 무관 정규화 (w6)

인코더가 순열불변이라 **작은 $W$ 에서 학습한 망을 큰 $W$ 에 그대로 쓸 수 있다.**
W=4 학습 → W=6 평가 갭이 0.121% 로 사전등록 기준 1% 를 크게 밑돈다
(`results/w6/DIGEST_w6.md` [4]).

다만 **정규화가 전이 방향을 정한다.**

| 정규화 | 위로 (4→6) | 아래로 (5→4) |
|---|---:|---:|
| 기존 — 점유를 $W$ 로 나눔 | **0.121%** | 0.431% |
| `WREF` — 고정 상수로 나눔 | 0.326% | **0.052%** |

`encode.WREF` / `net.WREF` 는 **기본값 `None` = 기존 동작**이다. w1~w5 결과가 전부 그
경로로 나왔으므로 건드리지 말 것. 켜려면 둘을 같은 값으로 함께 설정한다
(`tests/test_w6.py` 가 기본값과 carry 일관성을 못 박는다).

## 5.3 캐시 재고조사 (w6)

캐시 디렉터리 이름이 md5 라 어느 회차 것인지 눈으로 구분되지 않는다.

```bash
python scripts/cache_inventory.py                       # 분류만
python scripts/cache_inventory.py --purge OLD-NS OLD-W --record results/w6/cache_purge.json
```

현행 코드로 키를 다시 계산해 `KEEP-w5`(현행 인스턴스)를 식별하고, 나머지를
`OLD-W`(w2~w4) / `OLD-NS`(구 NMAX/SMAX) / `UNKNOWN`(중단된 `.tmp`)으로 가른다.
`--purge` 는 `KEEP-w5` 를 명시해도 거부한다. 캐시는 전부 코드로 재생성 가능하고
재생성 비용은 W=4 8초 / W=5 79초 / W=6 587초다.
